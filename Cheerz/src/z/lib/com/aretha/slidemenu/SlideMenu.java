...
import z.lib.com.aretha.slidemenu.utils.ScrollDetectors;
 ...
 * 
 */
// z.lib.com.aretha.slidemenu.SlideMenu
public class SlideMenu extends ViewGroup {
	...
	private ISlideMenuListener iSlideMenuListener;

	public void setiSlideMenuListener(ISlideMenuListener iSlideMenuListener) {
		this.iSlideMenuListener = iSlideMenuListener;
	}
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		if (KeyEvent.ACTION_UP == event.getAction()) {
			...
			case KeyEvent.KEYCODE_BACK:
				if (isOpen) {
					if (iSlideMenuListener != null && !iSlideMenuListener.canUseBackMenu()) {
						return true;
					}
					close(true);
					return true;
	...
	private void setCurrentOffset(int currentOffset) {
		...
		final int currentContentOffset = mCurrentContentOffset = Math.min((slideDirectionFlag & FLAG_DIRECTION_RIGHT) == FLAG_DIRECTION_RIGHT ? mContentBoundsRight : 0,
				Math.max(currentOffset, (slideDirectionFlag & FLAG_DIRECTION_LEFT) == FLAG_DIRECTION_LEFT ? mContentBoundsLeft : 0));
		...
		for (int index = 0; index < count; index++) {
			...
			case LayoutParams.ROLE_SECONDARY_MENU:
				measureChild(child, widthMeasureSpec,
						slideMode == MODE_SLIDE_WINDOW ? MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(heightMeasureSpec) - statusBarHeight, MeasureSpec.getMode(heightMeasureSpec))
								: heightMeasureSpec);
				break;
			}
	...
	protected final boolean canScrollHorizontally(View v, int dx, int x, int y) {
		if (v instanceof ViewGroup) {
			...
			for (int index = 0; index < childCount; index++) {
				...
				if (x + scrollX >= left && x + scrollX < child.getRight() && y + scrollY >= top && y + scrollY < child.getBottom() && View.VISIBLE == child.getVisibility()
						&& (ScrollDetectors.canScrollHorizontal(child, dx) || canScrollHorizontally(child, dx, x + scrollX - left, y + scrollY - top))) {
					return true;
				}
			}
		}
	...
	protected final boolean canScrollVertically(View v, int dy, int x, int y) {
		if (v instanceof ViewGroup) {
			...
			for (int index = 0; index < childCount; index++) {
				...
				final int top = child.getTop();
				if (x + scrollX >= left && x + scrollX < child.getRight() && y + scrollY >= top && y + scrollY < child.getBottom() && View.VISIBLE == child.getVisibility()
						&& (ScrollDetectors.canScrollVertical(child, dy) || canScrollVertically(child, dy, x + scrollX - left, y + scrollY - top))) {
					return true;
				}
			}
		}
