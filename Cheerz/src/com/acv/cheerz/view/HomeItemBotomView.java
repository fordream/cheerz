...
public class HomeItemBotomView extends LinearLayout {
	...
	public void loadUrl(JSONObject jsonObject) {
		...
		// DisplayImageOptions.Builder().showImageOnLoading(R.drawable.noimage).showImageForEmptyUri(R.drawable.noimage).showImageOnFail(R.drawable.noimage)
		// .cacheInMemory(true).cacheOnDisk(true).considerExifParams(true).displayer(new
	...
	public void setData(Cursor cursor, View.OnClickListener clickListenerLike) {
		tv1.setText(CheerzUtils.getString(cursor, "artist_name"));
		tv2.setText(CheerzUtils.getString(cursor, "status_vote"));
		tv3.setText(CheerzUtils.getString(cursor, "date"));
		tv4.setText(CheerzUtils.getString(cursor, "content"));
		findViewById(R.id.like).setOnClickListener(clickListenerLike);
	}
}